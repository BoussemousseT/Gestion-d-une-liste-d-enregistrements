...
function tableHTML($table)
{
    if ($table == []) {
        ...
        //affiche la table dans le navigateur web du client
        $html = '<table border="1">';

        //première rangée avec les th's
        $html .= '<thead><tr>';
        foreach ($table[0] as $un_nom_de_colonne => $une_valeur) {
...
/**
 * enregistre adress IP date et heure d'une visite
 */

function datePageAccueil()
{
    $record =  $_SERVER['REMOTE_ADDR'] . ' ' . date(DATE_RFC2822) . PHP_EOL;
 ...
 */
function Picture_Uploaded_Is_Valid($file_input, $Max_Size = 5000000)
{
